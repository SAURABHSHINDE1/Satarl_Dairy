            ...
            <Card>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 text-sm max-h-[65vh] overflow-y-auto">
                {[
                  ...
                  ['Tank No.', viewRecord.tank_no],
                  ['Milk Type', viewRecord.type_of_milk],
                  ['Quantity (L)', viewRecord.milk_quantity_l?.toFixed(2) ?? '—'],
                  ['Temperature (°C)', viewRecord.temp_celsius?.toFixed(1) ?? '—'],
                  ['Flavour / Taste', viewRecord.flavour_taste ?? '—'],
                  ...
                  ['SNF %', viewRecord.snf_percent?.toFixed(2) ?? '—'],
                  ['Efficiency %', viewRecord.efficiency_percent != null ? viewRecord.efficiency_percent.toFixed(2) : '—'],
                  ['Protein %', viewRecord.protein_percent?.toFixed(2) ?? '—'],
                  ['Electrolyte', viewRecord.electrolyte_condition ?? '—'],
                  ...
                  <div key={label} className="bg-secondary-50 rounded-lg p-3">
                    ...
                    <p className="font-medium text-text-primary text-sm capitalize">{value}</p>
                  </div>
                ))}
              </div>
